...
// The unified renderScheduleList function
const unifiedScheduleFunc = `function renderScheduleList() {
    const container = document.getElementById('schedule-days-container');
    if (!container) return;

    // Update M2 view mode buttons styling
...
// Replace the renderScheduleList function at line 1753
const firstRenderScheduleRegex = /function renderScheduleList\(\) \{[\s\S]*?renderScheduleCalendar\(\);\s*\}/;
if (appJs.match(firstRenderScheduleRegex)) {
    appJs = appJs.replace(firstRenderScheduleRegex, unifiedScheduleFunc);
    console.log('✓ Successfully replaced first renderScheduleList with Unified Multi-View function!');
} else {
    console.log('⚠ Could not match firstRenderScheduleRegex in app.js');
}

fs.writeFileSync(appJsPath, appJs, 'utf8');
console.log('Cleaned and unified schedule renderer!');
